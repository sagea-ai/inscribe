...
const installOllama = async () => {
  ...
  try {
    if (platform === "darwin" || platform === "linux") {
      ...
      await execa("curl -fsSL https://ollama.com/install.sh | sh", {
        shell: true,
      });
      ...
      spinner.warn(
        chalk.yellow("Ollama requires manual installation on Windows.")
      );
      console.log(
        chalk.cyan(
      ...
      console.log(
        chalk.cyan("After installation, please run `npm install` again.")
      );
      return false;
    } else {
...
const pullModel = async () => {
  const spinner = ora(
    "Pulling Ollama model (comethrusws/sage-reasoning:3b)..."
  ).start();
  try {
  ...
  try {
    if (platform === "darwin") {
      try {
        ...
        spinner.succeed(
          "PDF processing tools (poppler) installed successfully."
        );
        return true;
      } catch (brewError) {
        spinner.warn(
          chalk.yellow("Homebrew not found. Please install poppler manually.")
        );
        console.log(
          chalk.cyan(
      ...
      try {
        ...
        spinner.succeed(
          "PDF processing tools (poppler-utils) installed successfully."
        );
        return true;
      } catch (aptError) {
        try {
          ...
          try {
            ...
            spinner.warn(
              chalk.yellow(
                "Package manager not found. Please install poppler-utils manually."
              )
            );
            console.log(
              chalk.cyan("Ubuntu/Debian: sudo apt-get install poppler-utils")
            );
            console.log(
              chalk.cyan("CentOS/RHEL: sudo yum install poppler-utils")
            );
      ...
      spinner.warn(
        chalk.yellow(
          "PDF processing tools require manual installation on Windows."
        )
      );
    ...
    console.log(
      chalk.yellow(
        "Please install poppler manually for PDF processing capabilities."
      )
    );
    return false;
  }
...
const main = async () => {
  console.log(
    chalk.bold.cyan(
      "🚀 Setting up INSCRIBE - Paper to Code Implementation Tool\n"
    )
  );
  ...
  try {
    ...
    console.log(
      chalk.green("✓ PDF processing tools (pdftotext) are already installed.")
    );
  } catch (error) {
  ...
  console.log(
    `${ollamaInstalled ? chalk.green("✓") : chalk.red("✗")} Ollama: ${
      ollamaInstalled ? "Ready" : "Manual installation required"
    }`
  );
  console.log(
    `${
      pdfProcessingInstalled ? chalk.green("✓") : chalk.red("✗")
    } PDF Processing: ${
      pdfProcessingInstalled ? "Ready" : "Manual installation required"
    }`
  );
